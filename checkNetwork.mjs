...
export const checkHttpServing = async ({
    uniqName,
    url,
    requestInit,
    attempCount = 1,
    ...
    try {
        ...
        if (state.available) {
            ...
            await state.notifyWithThrottle({
                title: '服务异常',
                message: `《${uniqName}》服务异常: ${error.message} `,
            });
...
export const checkSSL = async ({ host, notifyIfDaysLessThan = 7, obtain }) => {
    ...
    await notify({
        title: 'SSL证书即将过期',
        message: `${host} 证书将在 ${res.daysRemaining} 天后过期，正在尝试自动更新证书`,
    });
    try {
        await obtain();
    } catch (error) {
        await notify({
            title: 'SSL证书更新失败',
            message: `${host} 证书更新失败: ${error.message}`,
        });
        return;
    }
