...
export const backup = async (path: string, space: string) => {
  const target = `/home/urie/hd1/backup/${space}`;
  const { stderr: err } = await $`sudo rsync_tmbackup.sh ${path} ${target}`;
  // await $`sudo rsync -aAXv ${path} /home/urie/hd1/backup/${space}`;
  const errMsg = await $`echo ${err} 2>&1 | grep -v warn`;
  if (errMsg) notifyMeErr(err);
  if (!fs.existsSync(``)) {
  }
};
